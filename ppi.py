...
def evaluate(feats, model, idx, subgraph, labels, loss_fcn):
    ...
    with torch.no_grad():
        ...
        score = f1_score(labels[:idx].data.cpu().numpy(),
                         predict,
                         average='micro')
...
loader = Data.DataLoader(dataset=idx,
                         batch_size=1,
                         shuffle=True,
                         num_workers=0)
...
def train():
    ...
    for step, batch in enumerate(loader):
        ...
        loss_train = loss_fcn(output[:train_nodes[batch]],
                              batch_label[:train_nodes[batch]])
        loss_train.backward()
        optimizer.step()
...
def validation():
    ...
    for batch in range(2):
        ...
        score, val_loss = evaluate(batch_feature, model, val_nodes[batch],
                                   batch_adj, batch_label, loss_fcn)
        loss_val += val_loss
        acc_val += score
    loss_val /= 2
    acc_val /= 2
...
def test():
    ...
    for batch in range(2):
        ...
        score, loss = evaluate(batch_feature, model, test_nodes[batch],
                               batch_adj, batch_label, loss_fcn)
        loss_test += loss
        acc_test += score
    acc_test /= 2
...
for epoch in range(args.epochs):
    ...
    if (epoch + 1) % 1 == 0:
        print('Epoch:{:04d}'.format(epoch + 1), 'train',
              'loss:{:.3f}'.format(loss_tra), '| val',
